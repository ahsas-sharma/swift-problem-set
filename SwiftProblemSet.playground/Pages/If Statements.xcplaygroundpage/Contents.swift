...
func checkForRelease(bugs: Int, music: Bool, levels: Int) {
    ...
    if bugs >= 10 {
        print("Squash some bugs to bring the count below 10.")
    }
    
    if !hasMusic {
        print("Music is food for the soul. Please feed it by adding some music.")
    }
    
    if numberOfLevels <= 5 {
        print("There should be more than 5 levels.")
    }
}
...
func checkTrainingStatus(name: String, bike: Bool, run: Bool) {
    ...
    if bike && run {
        ...
        print("\(name) is not ready. Unable to finish the running portion")
    } else if !bike && run {
        print("\(name) is not ready. Unable to finish the bike portion")
    } else {
        print("\(name) is not ready at all. Unable to finish both bike and running portions.")
    }
}
